...
D = CSV.read("./data/per_Load.csv", DataFrame, header = false);      # Please adjust the address to the current address
cor = D[:, floor(Int, first):floor(Int, last)]
cor_Load = Matrix(cor)

# Calling the corrections (10 percentiles or 10 scenarios) of the PV
D = CSV.read("./data/per_PV.csv", DataFrame, header = false);
cor = D[:, floor(Int, first):floor(Int, last)]
cor_PV = Matrix(cor)
...
D = CSV.read("./data/forecasted_Load_daily_matrix_new.csv", DataFrame, header = false);
...
D = CSV.read("./data/forecasted_PV_daily_matrix_new.csv", DataFrame, header = false);
...
D = CSV.read("./data/real_daily_matrix_Load_new.csv", DataFrame, header = false);
Load_ = D[Day_number, floor(Int, first):floor(Int, last)]
Load = Vector(Load_)
demand_ = Load' / 1000

D = CSV.read("./data/real_daily_matrix_PV_new.csv", DataFrame, header = false);
PV_ = D[PV_day_number, floor(Int, first):floor(Int, last)]
PV = Vector(PV_)
PV_Power_ = PV' / 1000
...
# customcustomSDDP modeling

model = customcustomSDDP.LinearPolicyGraph(
    ...
    @variables(sp, begin
        Peak, (customcustomSDDP.State, initial_value = 0)
        0 <= Es <= Cb, (customcustomSDDP.State, initial_value = Cb)
        grid
        0 <= grid_pos
    ...
    customcustomSDDP.parameterize(sp, support) do ω
...
# Train
customcustomSDDP.train(model, iteration_limit = 300, print_level = 1)



# Test
historical_sampler = customcustomSDDP.Historical(
    [
...
simulations = customcustomSDDP.simulate(
    model,
    1,
