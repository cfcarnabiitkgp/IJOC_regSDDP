...
PV_15_ = CSV.read("./data/IEEE123/PV_15.csv", DataFrame, header = false)
Load_15_ = CSV.read("./data/IEEE123/Load_15.csv", DataFrame, header = false)
...
for jj = 1:15                   #  receding forecasting (hourly-based; thus 15 iterations)
    ...
    D = CSV.read("./data/per_Load.csv", DataFrame, header = false)##1
    ...
    D = CSV.read("./data/per_PV.csv", DataFrame, header = false)##1
    cor = D[:, floor(Int, first):floor(Int, last)]
    cor_PV = Matrix(cor)
    ...
    for kk = 1:4  # 15-mins-based scheduling  (4 interval in hour)
        ...
        model = customSDDP.LinearPolicyGraph(
            ...
            Peak_recent
            @variables(sp, begin
                Peak, (customSDDP.State, initial_value = Peak_recent)
                0 <= Es <= Cb, (customSDDP.State, initial_value = Cb_recnt)
                grid
                0 <= grid_pos
                -1 <= u <= 0.4
                factor
            ...
            customSDDP.parameterize(sp, support) do ω
        ...
        # Train
        customSDDP.train(model, iteration_limit = 100, print_level = 1)


        # Test
        historical_sampler = customSDDP.Historical(
            [
        ...
        simulations = customSDDP.simulate(
            model,
            1,
            [:u, :Peak, :factor, :Es],
